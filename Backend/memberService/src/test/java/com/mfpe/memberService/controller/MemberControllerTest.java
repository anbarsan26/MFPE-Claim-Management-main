...
import com.mfpe.memberService.service.MemberServiceService;
...
public class MemberControllerTest {
	...
	public void testgetBills() {
    	...
    	when(memberServiceService.fetchBills("121")).thenReturn(new Bills("221","P2314", d3,65600,1200,d2));
...
//   	}
    ...
    @Test
   	...
   	public void testgetClaimStatus() {
       	...
       	when(authClient.getsValidity(anyString())).thenReturn(new ValidatingDTO(true));
       	when(memberServiceService.fetchClaimStatus("AAAA","token")).thenReturn(new ClaimStatusDTO("AAAA","Pending","Need More Action"));
       ...
       @Test
      	...
      	public void testgetClaimStatus_fails1() {
       	...
       	when(authClient.getsValidity(anyString())).thenReturn(new ValidatingDTO(true));
          	when(memberServiceService.fetchClaimStatus(anyString(), anyString())).thenThrow(InvalidClaimIdException.class);
       ...
       @Test
      	...
      	public void testgetClaimStatusOnUpdate() {
          	...
          	when(authClient.getsValidity(anyString())).thenReturn(new ValidatingDTO(true));
          	when(memberServiceService.fetchClaimDetails(claimDetails,"token")).thenReturn(new ClaimStatusDTO("AAAA","Pending","Need More Action"));
